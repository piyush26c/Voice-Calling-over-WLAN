...
/**
 * Connects to the server, then starts receiving messages. 
 * Creates a MicThread that sends microphone data to the server, and creates an instance of AudioThread 
 * for each user.
...
public class Client extends Thread {
    ...
    public void run() {
        try {
            ...
            for (;;) {
              //this infinite cycle checks for new data from the server, then sends it to the correct AudioChannel.
              //if needed, a new AudioChannel is created
                
                if (s.getInputStream().available() > 0) { 
                    /*
                        we got something from the server, used available method from InputStream 
                    */
